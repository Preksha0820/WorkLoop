...
import { Users, UserCheck, Trash2, RefreshCw, Search, ChevronLeft, ChevronRight, UserCog, ArrowRight } from 'lucide-react';
...
const EmployeeManagementDashboard = () => {
  ...
  const fetchEmployees = async (page = 1) => {
    setLoading(true);
    try {
      const response = await apiService.get(`/admin/employees?page=${page}&limit=${limit}`);
  ...
  const handleSwitchTeam = async () => {
    ...
    try {
      ...
      const teamLead = teamLeads.find(lead => lead.id === parseInt(newTeamLeadId));
      toast.success(`${selectedEmployee.name} switched to ${teamLead.name}'s team!`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between mt-8">
                    <div className="text-sm text-gray-600">
                      Showing {filteredEmployees.length} of {totalUsers} employees
                    </div>
                    <div className="flex items-center space-x-2">
                      <button
              ...
              <div className="grid gap-4">
                {filteredTeamLeads.length === 0 ? (
                  ...
                  filteredTeamLeads.map((lead) => (
                    <div key={lead.id} className="bg-gradient-to-r from-white to-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
      ...
      {showSwitchModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Switch Employee Team</h2>
            <div className="mb-6">
              <p className="text-gray-600 mb-4">
                Switch <strong>{selectedEmployee?.name}</strong> to a new team lead:
              </p>
              <select
                value={newTeamLeadId}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Delete Team Lead</h2>
            <div className="mb-6">
              <p className="text-gray-600 mb-4">
                Are you sure you want to delete <strong>{selectedTeamLead?.name}</strong>?
              </p>
              <div className="bg-red-50 p-4 rounded-xl">
                <p className="text-red-700 text-sm">
                  This action cannot be undone. All employees under this team lead will need to be reassigned.
                </p>
              </div>
            </div>
            <div className="flex space-x-4">
      ...
      {showRoleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Promote to Team Lead</h2>
            <div className="mb-6">
              <p className="text-gray-600 mb-4">
                Are you sure you want to promote <strong>{selectedEmployee?.name}</strong> to Team Lead?
              </p>
              <div className="bg-blue-50 p-4 rounded-xl">
                <p className="text-blue-700 text-sm">
                  This will change their role from Employee to Team Lead and they will gain additional permissions.
                </p>
              </div>
            </div>
            <div className="flex space-x-4">
